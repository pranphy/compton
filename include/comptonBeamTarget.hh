...
class comptonBeamTarget {
    ...
    public:
        comptonBeamTarget();
	virtual ~comptonBeamTarget();

	G4double GetEffLumin(SamplingType_t) const;

	comptonVertex SampleVertex(SamplingType_t);
    ...
    private:
	comptonMultScatt fMS;
    public:
        const comptonMultScatt& GetMultScatt() const { return fMS; }
    ...
    public:
        comptonBeamTarget_t GetBeamTargetIO() const {
          comptonBeamTarget_t bm;
          bm.x = fVer.x();
          bm.y = fVer.y();
          bm.z = fVer.z();
          bm.dx = fDir.x();
