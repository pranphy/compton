...
#ifndef __COMPTONTRACKRECONSTRUCT_HH
#define __COMPTONTRACKRECONSTRUCT_HH
...
public:
  comptonTrackReconstruct();
  virtual ~comptonTrackReconstruct();

  void AddHit(comptonGenericDetectorHit* aHit);

  G4int GetTrackHitSize() { return aTrackHit.size(); };
  void  PrintTrackInfo(){
    PrintHitInfo(aTrackHit);
  };
  void  PrintHitInfo(std::vector<comptonGenericDetectorHit*> aHitVec);
  G4int ReconstructTrack();

  std::vector <comptonGenericDetectorHit*> GetTrack(){return aTrackHit;};
...
private:
  ...
  std::vector<comptonGenericDetectorHit*> aTrackHit;
  std::vector<comptonGenericDetectorHit*> aTrackHitOrg; // original
