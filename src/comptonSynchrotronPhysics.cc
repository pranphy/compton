...
comptonSynchrotronPhysics::comptonSynchrotronPhysics(G4int verbose, const G4String& name){
    SetVerboseLevel(verbose);
    SetPhysicsName(name);
    fDecayPhysics = new G4DecayPhysics();
    fSRType = true; // true for G4SynchrotronRadiation, false for G4SynchrotronRadiationInMat
}

comptonSynchrotronPhysics::~comptonSynchrotronPhysics(){
    delete fDecayPhysics;
}
void comptonSynchrotronPhysics::ConstructProcess()
{
    ConstructEM();
    fDecayPhysics->ConstructProcess();
}

void comptonSynchrotronPhysics::ConstructParticle(){
    fDecayPhysics->ConstructParticle();
}

void comptonSynchrotronPhysics::ConstructEM(){
...
void comptonSynchrotronPhysics::ConstructSync()
{
    auto particleIterator=GetParticleIterator();
    particleIterator->reset();
    G4SynchrotronRadiation* fSync = new G4SynchrotronRadiation();
