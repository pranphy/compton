...
comptonGenpElastic::comptonGenpElastic()
: comptonVEventGen("elastic") {
    fTh_min =     0.1*deg;
    fTh_max =     2.0*deg;
...
void comptonGenpElastic::SamplePhysics(comptonVertex *vert, comptonEvent *evt){
    // Generate ep event
...
G4double comptonGenpElastic::RadProfile(G4double eloss, G4double btt){
...
G4double comptonGenpElastic::EnergNumInt(G4double btt, G4double a0, G4double b0){
    const int nbin = 1000;
    double sum = 0.0;
    double bremcut = fBeamTarg->fEnergyCut;
