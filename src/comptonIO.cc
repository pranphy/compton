...
ClassImp(comptonSeed_t)

// Singleton
comptonIO* comptonIO::gInstance = 0;
comptonIO* comptonIO::GetInstance(const G4String& outputfile) {
  if (gInstance == 0) {
    gInstance = new comptonIO(outputfile);
  }
  return gInstance;
}

comptonIO::comptonIO(const G4String& outputfile)
...
void comptonIO::WriteTree()
{
    ...
    comptonRun::GetRunData()->Write("run_data", TObject::kOverwrite);
...
void comptonIO::SetEventData(const comptonEvent *ev)
{
...
// GenericDetectorHit
void comptonIO::AddGenericDetectorHit(comptonGenericDetectorHit *hit)
{
    fGenDetHit.push_back(hit->GetGenericDetectorHitIO());
}

// GenericDetectorSum
void comptonIO::AddGenericDetectorSum(comptonGenericDetectorSum *hit){
    fGenDetSum.push_back(hit->GetGenericDetectorSumIO());
}

/*---------------------------------------------------------------------------------*/

void comptonIO::GrabGDMLFiles(G4String fn)
{
    // Reset list
...
void comptonIO::SearchGDMLforFiles(G4String fn)
{
...
void comptonIO::TraverseChildren( xercesc::DOMElement *thisel )
{
