...
comptonGenPion::comptonGenPion()
: comptonVEventGen("pion"),
  fPionType(kPiMinus) {
    ...
    fThisGenMessenger.DeclareMethod("piontype",&comptonGenPion::SetPionTypeByString_Deprecated,"Generate pion type");
    fThisGenMessenger.DeclareMethod("settype",&comptonGenPion::SetPionTypeByString,"Generate pion type");
}

comptonGenPion::~comptonGenPion() { }

void comptonGenPion::SamplePhysics(comptonVertex *vert, comptonEvent *evt)
{
    // Generate Pion event
    ...
    //double beamE = comptonBeamTarget::GetBeamTarget()->fBeamEnergy;
    ...
    if (!(pf >= 0.0) || !(pf <= beamE)) {
      G4cout << "comptonGenPion::SamplePhysics: ERROR"
...
double comptonGenPion::wiser_sigma(
	double Ebeam, // GeV
	double pf,    // GeV
	double thf,   // rad
	double rad_len, // 1
	int type)
{
	...
	TF1 *wiserfit = new TF1("wiserfit", comptonGenPion::wiserfit, E_gamma_min, Ebeam, 5, 1, TF1::EAddToList::kNo);
        #else
	TF1 *wiserfit = new TF1("wiserfit", comptonGenPion::wiserfit, E_gamma_min, Ebeam, 5);
        #endif
...
double comptonGenPion::wiserfit(double *x, double *par)
{
