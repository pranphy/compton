 ...
 */
std::vector<comptonEventParticle_t> comptonEvent::GetEventParticleIO() const {
  std::vector<comptonEventParticle_t> parts;
  for (size_t idx = 0; idx < fPartType.size(); idx++) {
    comptonEventParticle_t part;
    ...
    if(trajectoryContainer==0){
      ...
      for(decltype(n) i = 0; i < n; i++) {
        ...
        if(((*trajectoryContainer)[i]->GetParentID() == 0) && ((*trajectoryContainer)[i]->GetTrackID() == G4int(idx+1))){
          //Store each point in the container in the comptonEventParticle_t structure
 ...
 */
comptonEvent_t comptonEvent::GetEventIO() const {
  comptonEvent_t ev;
...
void comptonEvent::ProduceNewParticle(G4ThreeVector pos, G4PrimaryParticle* particle)
{
...
void comptonEvent::ProduceNewParticle(G4ThreeVector pos, G4ThreeVector mom, G4String name, G4ThreeVector spin )
{
    fPartPos.push_back(pos);
    fPartMom.push_back(mom);
    fPartSpin.push_back(spin);
...
void comptonEvent::UndoLastParticle(){
    fPartPos.pop_back();
    fPartMom.pop_back();
    fPartSpin.pop_back();
