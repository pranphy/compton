...
namespace { G4Mutex comptonBeamTargetMutex = G4MUTEX_INITIALIZER; }

// Initialize static geometry objects
G4String comptonBeamTarget::fActiveTargetMotherName = "LH2";
G4String comptonBeamTarget::fActiveTargetVolumeName = "LH2";
size_t comptonBeamTarget::fActiveTargetMother = 0;
size_t comptonBeamTarget::fActiveTargetVolume = 0;
std::vector<std::pair<G4VPhysicalVolume*,G4String>> comptonBeamTarget::fTargetMothers;
std::vector<std::vector<std::pair<G4VPhysicalVolume*,G4String>>> comptonBeamTarget::fTargetVolumes;

G4double comptonBeamTarget::fActiveTargetEffectiveLength  = -1e9;
G4double comptonBeamTarget::fMotherTargetAbsolutePosition = -1e9;
G4double comptonBeamTarget::fTotalTargetEffectiveLength = 0.0;
  ...
  fdTh(0.0),fdPh(0.0),fCorrTh(0.0),fCorrPh(0.0)
{
    ...
    fTargetMessenger.DeclareMethod("mother",&comptonBeamTarget::SetActiveTargetMother,"Set target mother name").SetStates(G4State_Idle);
    fTargetMessenger.DeclareMethod("volume",&comptonBeamTarget::SetActiveTargetVolume,"Set target volume name").SetStates(G4State_Idle);
    fTargetMessenger.DeclareMethod("print",&comptonBeamTarget::PrintTargetInfo).SetStates(G4State_Idle);
}
...
G4double comptonBeamTarget::GetEffLumin(SamplingType_t sampling_type) const
{
...
void comptonBeamTarget::PrintTargetInfo()
{
...
void comptonBeamTarget::UpdateInfo()
{
...
comptonVertex comptonBeamTarget::SampleVertex(SamplingType_t sampling_type)
{
    // Create vertex
    comptonVertex vertex;
