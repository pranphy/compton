...
TFile* comptonGenExternal::fFile = 0;
TTree* comptonGenExternal::fTree = 0;
comptonEvent_t* comptonGenExternal::fEvent = 0;
std::vector<comptonGenericDetectorHit_t>* comptonGenExternal::fHit = 0;
Int_t comptonGenExternal::fEntry = 0;
Int_t comptonGenExternal::fEntries = 0;

comptonGenExternal::comptonGenExternal()
  ...
  fzOffset(0), fDetectorID(28), fLoopID(1)
{
  ...
  fThisGenMessenger.DeclareMethod("detid",&comptonGenExternal::SetGenExternalDetID,"External generator detector ID");
  fThisGenMessenger.DeclareMethod("startEvent",&comptonGenExternal::SetGenExternalEntry,"External generator starting event: -1 starts random,  n starts at n (default n=0)");
}
...
void comptonGenExternal::SetGenExternalFile(G4String& filename)
{
  G4AutoLock inFileLock(&inFileMutex);
...
void comptonGenExternal::SamplePhysics(comptonVertex* /* vert */, comptonEvent* evt)
{
  G4AutoLock inFileLock(&inFileMutex);
  ...
  do {
    ...
    for (size_t i = 0; i < fHit->size(); i++) {
      ...
      comptonGenericDetectorHit_t hit = fHit->at(i);
